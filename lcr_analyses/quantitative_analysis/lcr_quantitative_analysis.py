#!/usr/bin/env python3
"""Phase 2.4.1 (v2.1): LCR diversity across RNA-target classes within the RBP set.

Pipeline: load -> join -> per method { categorical + continuous, LCR + protein
level } -> one Excel workbook per method + figures. Output manual:
results_guide_phase2_4_1.md.

Design invariants:
- Reference design: ONE-VS-REST (each class vs. all other classes pooled).
- Statistics: Fisher exact + log2 odds ratio (categorical cells);
  Kruskal-Wallis omnibus + per-class Cliff's delta (continuous).
- BH-FDR within each analysis block x method; alpha = 0.05.
- Binary (two-valued) features report one arm only -- the second row is an
  exact mirror (negated OR, identical p) and is omitted.
- Column order is fixed by order_cols(): identity -> counts -> rates ->
  effect size -> statistics -> flags.
- Protein level carries the claims; LCR level is descriptive.

v2.1 review fixes: fixed column order across all sheets; mirror rows removed;
FDR applied per analysis block (not per merged sheet); weighted-path no-op
removed. Data-integration fixes from v1 retained.
"""
...
def extract_accession(x: str) -> str:
    """UniProt accession from a composite ID: text before the first '|'."""
    text = str(x).strip()
    if "|" in text:
    ...
    for col in merged.columns:
        if col not in INTERVAL_KEYS:
            try:
                ...
                pass  # textual columns stay as-is
    return merged
...
def main() -> None:
    ...
    parser.add_argument("--annotations",
                        default=r"lcr_analyses\pc_properties\lcr_annotations.xlsx")
    parser.add_argument("--measurements",
                        default=r"lcr_analyses\pc_properties\lcr_features.xlsx")
    parser.add_argument("--master",
                        default=r"datasets\combined_rbp_pfam38_rbpdb_modomics_uniprot.xlsx")
    parser.add_argument("--outdir", default=r"lcr_analyses\quantitative_analysis\results_v2")

...
def encode_position(ann: pd.DataFrame, proteins: pd.DataFrame) -> pd.DataFrame:
    ...
    ann = ann[ann["position"] != "unclassifiednopfam"].copy()
    present = (
...
def gower_mixed_distance(
    ...
    """Simple Gower-style distance for mixed binary/categorical/continuous data."""
    n = df.shape[0]
    dist = np.zeros((n, n), dtype=float)
    w_sum = 0.0
    # weight each feature block equally
    w_bin = 1.0 / max(len(binary_cols), 1)
    w_cat = 1.0 / max(len(categorical_cols), 1)
    w_con = 1.0 / max(len(continuous_cols), 1)
    ...
    # categorical: simple mismatch
    if categorical_cols:
        Xc = df[categorical_cols].to_numpy()
        dc = np.zeros((n, n), dtype=float)
        for j in range(Xc.shape[1]):
            col = Xc[:, j]
            dc += (col[:, None] != col[None, :]).astype(float)
        dc /= Xc.shape[1]
        dist += w_cat * dc
        w_sum += w_cat
